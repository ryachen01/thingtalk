...
class Scope {
    constructor(scope) {
        if (scope) {
            this._globalScope = Object.assign({}, scope._globalScope);
            this._scope = Object.assign({}, scope._scope);
            this.$has_event = scope.$has_event;
            this._lambda_args = Object.assign({}, scope._lambda_args);
        } else {
            this._globalScope = {};
            this.clean();
        }
    ...
    has(name) {
        return name in this._scope || name in this._lambda_args || name in this._globalScope;
    }
    ...
    get(name) {
        return this._scope[name] || this._lambda_args[name] || this._globalScope[name];
    }
...
function loadSchema(schemas, classes, prim, primType, useMeta, isMeta = false) {
    ...
    return Utils.getSchemaForSelector(schemas, prim.selector.kind, prim.channel, primType, useMeta, classes, isMeta);
}
...
function resolveScalarExpression(ast, schema, scope, schemas, classes, useMeta, isMeta = false) {
    log('Type check scalar expression');
    if (ast.isBoolean) {
        typeCheckFilter(ast.value, schema, scope, schemas, classes, useMeta, isMeta);
        return Type.Boolean;
    }
    if (ast.isPrimary) {
    ...
    if (ast.isDerived) {
        let operands = ast.operands.map((o) => resolveScalarExpression(o, schema, scope, schemas, classes, useMeta, isMeta));
...
function typeCheckFilter(ast, schema, scope, schemas, classes, useMeta, isMeta = false) {
    ...
    return (async function recursiveHelper(ast) {
        ...
        if (ast.isAtom) {
            ...
            if (ast.schema === null)
                ast.schema = await loadSchema(schemas, classes, ast, 'query', useMeta, isMeta);
            await typeCheckInputArgs(ast, ast.schema, scope, classes, isMeta);
            addRequiredInputParamsInvocation(ast, null);
            return typeCheckFilter(ast.filter, ast.schema, scope, schemas, classes, useMeta, isMeta);
        }
    })(ast);
}
...
function typeCheckComputation(ast, innerSchema, scope, schemas, classes, useMeta, isMeta = false) {
    let name = ast.alias ? ast.alias : 'result';
    let type = resolveScalarExpression(ast.expression, innerSchema, scope, schemas, classes, useMeta, isMeta);
...
function typeCheckInputArgs(ast, schema, scope, classes, isMeta = false) {
    if (!ast.isVarRef && !ast.isJoin) {
        if (ast.selector.kind in classes && !isMeta)
            ast.__effectiveSelector = Ast.Selector.Device(classes[ast.selector.kind].extends[0], ast.selector.id, ast.selector.principal);
        else
...
async function typeCheckTable(ast, schemas, scope, classes, useMeta = false, isMeta = false) {
    if (ast.isVarRef) {
        ast.schema = await typeCheckInputArgs(ast, ast.schema, scope, classes, isMeta);
        scope.addAll(ast.schema.out);
    } else if (ast.isInvocation) {
        ast.schema = await typeCheckInputArgs(ast.invocation, ast.invocation.schema, scope, classes, isMeta);
        scope.addAll(ast.schema.out);
    } else if (ast.isFilter) {
        await typeCheckTable(ast.table, schemas, scope, classes, useMeta, isMeta);
        ast.schema = ast.table.schema;
        await typeCheckFilter(ast.filter, ast.schema, scope, schemas, classes, useMeta, isMeta);
    ...
    } else if (ast.isArgMinMax) {
        await typeCheckTable(ast.table, schemas, scope, classes, useMeta, isMeta);
        typeCheckArgMinMax(ast);
    } else if (ast.isJoin) {
        let leftscope = new Scope(scope);
        let rightscope = new Scope(scope);

        await typeCheckTable(ast.lhs, schemas, leftscope, classes, useMeta, isMeta);
        await typeCheckTable(ast.rhs, schemas, rightscope, classes, useMeta, isMeta);
        leftscope.$has_event = true;
        await typeCheckInputArgs(ast, ast.rhs.schema, leftscope, classes, isMeta);
        ...
        await typeCheckStream(ast.stream, schemas, scope, classes, useMeta, isMeta);
        ...
        if (ast.isHistory && (!typeForValue(ast.base, scope).isDate
                ...
                || typeForValue(ast.delta, scope).unit !== 'ms'))
            throw new TypeError('Invalid time range');
        await typeCheckStream(ast.table, schemas, scope, classes, useMeta, isMeta);
        ast.schema = ast.table.schema;
    } else if (ast.isCompute) {
        await typeCheckTable(ast.table, schemas, scope, classes, useMeta);
        typeCheckComputation(ast, ast.table.schema, scope, schemas, classes, useMeta, isMeta);
    } else {
        throw new Error('Not Implemented');
    }
}

async function typeCheckStream(ast, schemas, scope, classes, useMeta = false, isMeta = false) {
    if (ast.isVarRef) {
        ast.schema = await typeCheckInputArgs(ast, ast.schema, scope, classes, isMeta);
        scope.addAll(ast.schema.out);
        ...
        await typeCheckStream(ast.stream, schemas, leftscope, classes, useMeta, isMeta);
        await typeCheckTable(ast.table, schemas, rightscope, classes, useMeta, isMeta);
        leftscope.$has_event = true;
        await typeCheckInputArgs(ast, ast.table.schema, leftscope, classes, isMeta);
...
async function typeCheckAction(ast, schemas, scope, classes, useMeta, isMeta = false) {
    ast.schema = await typeCheckInputArgs(ast.invocation, ast.invocation.schema, scope, classes, isMeta);
}
...
function loadAllSchemas(ast, schemas, scope, classes, useMeta, isMeta = false) {
    return Promise.all(Array.from(ast.iteratePrimitives(true)).map(async ([primType, prim]) => {
        ...
        if (prim.isVarRef) {
            ...
            if (!(schema instanceof Ast.ExpressionSignature) || schema.functionType !== primType)
                throw new TypeError(`Variable ${ast.name} does not name a ${primType}`);
        } else {
            schema = await loadSchema(schemas, classes, prim, primType, useMeta, isMeta);
        }
        prim.schema = schema;
    }));
}

async function typeCheckClass(klass, schemas) {
...
async function typeCheckDeclaration(ast, schemas, scope, classes, useMeta, isMeta = false) {
    if (ast.type === 'program') {
        await loadAllSchemas(ast, schemas, scope, classes, useMeta, isMeta);
        if (Object.keys(ast.args).length !== 0)
            throw new TypeError(`Arguments are not allowed for program.`);
        await typeCheckProgram(ast.value, schemas, useMeta, classes, isMeta);
    } else {
        ...
        switch (ast.type) {
            case 'stream':
                ...
                await typeCheckStream(ast.value, schemas, scope, classes, useMeta, isMeta);
                break;
            case 'query':
                addRequiredInputParamsTable(ast.value, null);
                await typeCheckTable(ast.value, schemas, scope, classes, useMeta, isMeta);
                break;
            case 'action':
                addRequiredInputParamsAction(ast.value);
                await typeCheckAction(ast.value, schemas, scope, classes, useMeta, isMeta);
                break;
            default:
                throw new TypeError(`Invalid declaration type ${ast.type}`);
        }

        const schema = updateLambdaArgs(ast);
        scope.addGlobal(ast.name, schema);
    }
}

async function typeCheckRule(ast, schemas, scope, classes, useMeta = false, isMeta = false) {
    await loadAllSchemas(ast, schemas, scope, classes, useMeta, isMeta);
    ...
    await Promise.all(
        ast.actions.map((action) => typeCheckAction(action, schemas, scope, classes, useMeta, isMeta)));
}

async function typeCheckProgram(ast, schemas, useMeta = false, classes = {}, isMeta = false) {
    ...
    for (let klass of ast.classes)
        await typeCheckClass(klass, schemas);
    for (let decl of ast.declarations) {
        scope.clean();
        await typeCheckDeclaration(decl, schemas, scope, classes, useMeta, isMeta);
    }
    for (let decl of ast.rules) {
        scope.clean();
        await typeCheckRule(decl, schemas, scope, classes, useMeta, isMeta);
    }
}
...
async function typeCheckExample(ex, schemas, classes = {}, getMeta = false) {
    await typeCheckDeclaration(ex, schemas, new Scope(), classes, getMeta, true);
    if (!Array.isArray(ex.utterances))
