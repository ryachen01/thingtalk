...
const get_label_parameter_triplet = `?person wdt:%s ?item%d.\n`;

//for some values(e.g. numbers) there is no wikidata represntation
const no_label_triplet = `?person wdt:%s %s.\n`;

//for some values(e.g. numbers) there is no wikidata represntation

module.exports = {
  program_to_sparql(program) {
    const filters = program["rules"][0]["table"]["table"]["filter"];
    const schema = program["rules"][0]["table"]["table"]["schema"];
  ...
  filter_to_triplet(filter_input, schema_input, index) {
    ...
    /*
    this is a special case where the user filters for point in time of a certain event
    this requires a modified format for the query
    */
    ...
    if (isUnique) {
      ...
      value = filter_input["value"]["display"];
      return [no_label_triplet.format(parameter, value), false];
    } else {
      return [
  ...
  array_to_triplets(input_array, parameter, index, type) {
    ...
    for (var i = 0; i < input_array.length; i++) {
      ...
      if (type.includes("Entity")) value = input_array[i]["display"];
