...
module.exports = {
    ...
    if (filters["operands"] !== undefined) {
      ...
      filter_list.map((filter) => {
        ...
        if (`${type}`.includes("Array")) {
          ...
          let triplets = this.array_to_triplets(value, parameter, counter);
      ...
      if (`${type}`.includes("Array")) {
        ...
        let triplets = this.array_to_triplets(value, parameter, 1);
        for (var i = 0; i < triplets[0].length; i++) {
          label_triplets += triplets[0][i];
          parameter_triplets += triplets[1][i];
        }
  ...
  array_to_triplets(input_array, parameter, index) {
    let label_triplets = [];
    let parameter_triplets = [];
